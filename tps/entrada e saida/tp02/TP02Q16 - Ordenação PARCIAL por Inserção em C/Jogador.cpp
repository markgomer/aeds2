...
void insercao(Jogador *array, int n, int k){
  //insercao nos k primeiros
  for (int i = 1; i < k+1; i++) {
    Jogador tmp = array[i];
    int j = i - 1;
  ...
  //comparar os elementos fora do k com o ultimo dentro do k (o maior)
  for(int i = k + 1; i < n; i++) {
    //se for menor, trocar com o ultimo
    if (
      numComparacoes++ != 0 &&
      array[k].anoNascimento > array[i].anoNascimento || (
        numComparacoes++ != 0 &&
        array[k].anoNascimento == array[i].anoNascimento &&
        numComparacoes++ != 0 &&
        strcmp(array[k].nome, array[i].nome) > 0
      )
    ) {
      swap(&array[k], &array[i]);
      //aplicar insertionsort de novo
      insercao(array, n, k);
    } 
  }
}
