...
public class Jogador {
  ...
  public static void selectionSort(Jogador array[], int k) {
    int n = array.length;
    for (int i = 0; i < k; i++) {
      int menor = i;
      for (int j = (i + 1); j < n; j++){
        numComparacoes++;
        if (array[menor].nome.compareTo(array[j].nome) > 0) {
          menor = j;
        }
      }
      swap(array, menor, i);
  ...
  public static void main(String[] args) {
    ...
    selectionSort(jogador, k);  // ordena vetor pelo nome dos jogadores
    ...
    Arq.openWriteClose("matricula_selecao_parcial.txt", conteudo);
  }
}
